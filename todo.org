...
* Currently doing / Next up
  ...
  - make a dedicated batch for alpha blended static objects (windows, shadows)

  - look at extra pivot data, so i can crop images in sheets, and reuse stair images for the 4 parts

  - animating bodyparts

  - have 'dynamic architecture' animatable (door open and closes, escalator running)
