...
NOTE:
On the rpi GL_OES_mapbuffer is supported it seems
...
* Currently doing / Upcoming
  finish hot reloading: get all memory addressed via PUSH_STRUCTs et al.
  reinterpret that memory via the gamelibrary (game_update_and_render func)
  debug rendering, colored lines, colored rectangles (filled and unfilled)
