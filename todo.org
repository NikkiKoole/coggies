* Currently doing
  - [ ] do all memory usage by pushstrcut et all (no predefined arrays no more in permanent)
  - [ ] use permanent for the grid (using scratch now, dont know why it doenst work)
  - [ ] clean up path finder grid after path is found (dirty list)
  - [ ] stairs/ladders
