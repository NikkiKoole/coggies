...
* Currently doing / Next up
  - animating body parts
    There's quite a bit to this, lets list it out.
    - make a texture atlas and map file from ASE files, can use aseprite cli interface for that.
    - also in that ASE file i will need to place anchor points and save them out.
    - the anchor points shouldn't be visible in the texture atlas, I just need their locations
    - the json needs to be transformed into a binary file format and/or into a H/C file i can link to.
    - to summarize: I need some data structure which combines: originalSource size, currentSource size, pivot position, anchor point(s) position(s). PER body part.
    - I need a separate tool that can combine the parts into frames, to check if everything is working as intended.
    - perhaps this tool will be useful if I choose to use single framed actors instead of multi part sprites (seeing how heavy it is on RPI)
    - about that: maybe I will need some render to texture experiment to see if i can generate a set of single frames for some random actor
    - BTW, the parts we are talking about are: legs, torso, left-arm, right-arm, head, hair(?), hat(?), mouth(?)
    - so at maximum we need to use 8x the amount of buffers, minimum 5x, seeing how on RPI it already struggles to display 1000 actors ATM, this means this number will be even allot lower (150, 200)
    - for some projects it might very well be OK to just offload all combined sprite data into a huge single texture, when I don't need too many animations and not too many actors maybe.
    - so I think it would be wise to have a few options of displaying actors eventually: ['with single frames', 'with 5,8 combined parts, 'render combined  into temporary single texture (fbo off screen)']
    - then there's the whole way of describing an animation in body parts to solve, and making use of the extra options using parts offer, like random stances on torso vs arm or legs etc.


  - havent really done anything with this for a good few weeks/months,
    looking at it the simplest way back in is:
    start using the json and generated texture atlasses for blocks first
    then look into the characters
    I really should have different ways of combining, for example for  the escalator work I could really just have a head and the rest of a body (meaning just 2 draws per character instead of 8)
    For something else the fully separated one migh be better, so I have to support both.
    The simpler thing would be to start with just the 2 part draw (heads and body) then we can grow from there when its needed.
...
* Todo
- [ ] clean up the GLKMath code, preferably into 1 header file and get rid of all the unneeded cruft
