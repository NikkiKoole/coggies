...
void combine_pixels_pivot_anchor(TextureAtlasLayerValues *pixel_layer, TextureAtlasLayerValues *all_data, CombinedSets *combined, char *prefix){
    int i, j, k;
    int index = 0;
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 4) {
        ...
        if (use == Bodies) {
            ...
            combine_pixels_pivot_anchor(&pixel_layer, &all_data, &combined, "BP");
        } else if (use == Blocks) {
