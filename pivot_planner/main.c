    ...
    for (i = 0; i< all_data->total;i++) {
        ...
        if (all_data->data[i].child_count > 0) {
            ...
            if (pivot && anchor_1 && pixel) {
                ...
                printf("%s %s %s, frames #%d \n", item->file_name, item->group_name, item->layer_name, pivot->frame_count);
...
int main() {
    const char* all_str = readFile("./all.json");
    const char* pix_str = readFile("./pixel.json");
    ...
    if (all < 0) {
        printf("failed to parse JSON, maybe enlarge the buffer above? %d\n", all);
    }
    if (pixels < 0) {
        printf("failed to parse JSON, maybe enlarge the buffer above? %d\n", pixels);
    }
    ...
    combine_all_and_pixel(&pixel_layer, &all_data, &combined);
